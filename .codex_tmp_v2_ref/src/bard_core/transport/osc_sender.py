...
from ..contracts import MOOD_LABELS, StoryFragment
...
class ProcessingOscStream:
    ...
    def send(self, fragment: StoryFragment, *, final: bool = False) -> None:
        ...
        time.sleep(0.03)
        _send_mood_cues(self.client, fragment)
...
def _send_mood_cues(client, fragment: StoryFragment) -> None:
    for cue in fragment.mood_cues:
        mood = str(cue.get("mood") or "").upper().strip()
        if mood not in MOOD_LABELS:
            continue
        try:
            start_s = float(cue.get("start_s"))
            end_s = float(cue.get("end_s"))
        except (TypeError, ValueError):
            continue
        if end_s <= start_s:
            continue
        client.send_message("/mood", [int(fragment.id), mood, start_s, end_s])
        time.sleep(0.01)
