...
def run_sequential_pipeline(
    ...
    *,
    ...
    for index, chunk in enumerate(chunks):
        ...
        require_ready_image = generate_images and index == 0
        _validate_scene_ready(fragment, require_image=require_ready_image)
        if generate_images and not require_ready_image and not _scene_has_ready_image(fragment):
            print(
                f"Warning: Story scene {fragment.id} has no usable image; continuing with text/music visuals."
            )
