...
from .contracts import (
    ...
    PipelineResult,
    StoryFragment,
    mood_cues_from_music_segments,
    music_segments_from_json,
    story_fragments_from_json,
    write_json,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "send-osc":
        story_json = Path(args.story_json).expanduser()
        fragments = story_fragments_from_json(story_json)
        _attach_adjacent_music_mood_cues(fragments, story_json)
        send_fragments_to_processing(
            fragments=fragments,
...
def _attach_adjacent_music_mood_cues(fragments: list[StoryFragment], story_json: Path) -> None:
    music_path = story_json.parent / "music_segments.json"
    if not music_path.exists():
        return
    music_segments = music_segments_from_json(music_path)
    if not music_segments:
        return
    for fragment in fragments:
        if fragment.mood_cues:
            continue
        fragment.mood_cues = mood_cues_from_music_segments(
            music_segments,
