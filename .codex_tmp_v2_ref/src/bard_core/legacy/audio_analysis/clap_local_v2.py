...
def feature_tensor(features):
    """Return the tensor from CLAP feature outputs across transformers versions."""
    ...
    if isinstance(features, (tuple, list)) and features:
        return feature_tensor(features[0])
...
def compute_label_embeddings_from_labelbank(
    ...
    for start in range(0, len(prompts), batch_size):
        ...
        with torch.no_grad():
            emb = feature_tensor(model.get_text_features(**text_inputs))
...
def run_embeddings(
    ...
    if labelbank_json:
        ...
        text_inputs = {k: v.to(device) for k, v in text_inputs.items()}
        with torch.no_grad():
            text_emb = feature_tensor(model.get_text_features(**text_inputs))
    ...
    for (start, end, chunk) in chunks:
        ...
        audio_inputs = processor(audio=chunk, sampling_rate=sr, return_tensors="pt")
        audio_inputs = {k: v.to(device) for k, v in audio_inputs.items()}

        with torch.no_grad():
            audio_emb = feature_tensor(model.get_audio_features(**audio_inputs))
