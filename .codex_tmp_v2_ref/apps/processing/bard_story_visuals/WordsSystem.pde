class WordsSystem {
  ...
  void beginScene(String textValue, float durationSeconds) {
    ...
    for (int index = 0; index < sentences.size(); index++) {
      ...
      // Reserve a stable-reading interval while keeping each sentence inside its original slot.
      int stableMs = constrain(round(slotMs * 0.40f), 1800, 5200); // tempo di lettura in cui le parole stan ferme
