class OscHandler {
  ...
  void handleMessage(OscMessage message) {
    ...
    if (message.checkAddrPattern("/mood")) {
      int segmentId = message.get(0).intValue();
      String mood = message.get(1).stringValue();
      float startValue = message.get(2).floatValue();
      float endValue = message.get(3).floatValue();

      Segmento segment = director.findSegment(segmentId);
      if (segment != null) {
        segment.addMoodCue(mood, startValue, endValue);
        println(">>> Received mood cue for segment " + segmentId + ": " + mood + " " + startValue + "-" + endValue);
      } else {
        println(">>> Ignored mood cue, segment not found: " + segmentId);
      }
      return;
    }
