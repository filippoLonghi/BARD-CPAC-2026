...
- `story/gemini_story.py`: persistent audio-driven symbolic adventure bible, continuity state, and three visual assets per beat.
- `pipeline_sequential.py`: uploaded-file stream simulation and planned-duration live foundation.
- `images/`: optional image-keyframe providers for Replicate FLUX, Gemini image models through the legacy `imagen` provider name, and Openverse retrieval.
