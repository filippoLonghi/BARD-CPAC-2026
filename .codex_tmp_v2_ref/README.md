...
Story generation now chooses a deterministic audio-driven world profile before the story bible is
created. The profile uses extracted descriptors such as valence, arousal, tension, instruments,
texture, dynamics, rhythm, genres, and the music prompt to steer the story toward settings such as
cyberpunk rooftops, moon archives, storm airships, radio towers, festival harbors, or desert caravans
instead of always falling back to woodland imagery.
...
Then try generated images with FLUX or the backwards-compatible `imagen` provider. The provider name
is kept for old commands, but it now uses Gemini image models by default:
...
Generated subject and symbol assets are post-processed in Python into transparent PNGs before
Processing receives them. See [docs/image_generation.md](docs/image_generation.md) for API keys,
Gemini image model settings, cutouts, output files, and debugging.
...
- [docs/image_generation.md](docs/image_generation.md): FLUX, Gemini image models, Openverse, scene cards, alpha cutouts, and Processing image OSC.
