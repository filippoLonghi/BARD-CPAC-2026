...
from bard_core.pipeline_sequential import _processing_visible_path, _scene_has_ready_image, _validate_scene_ready
...
class SequentialPipelineTests(TestCase):
    ...
    def test_scene_readiness_blocks_blank_start_and_accepts_real_image(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertTrue(_scene_has_ready_image(fragment))

    def test_scene_readiness_detects_missing_image_without_requiring_it(self) -> None:
        fragment = StoryFragment(id=2, mood="CALM", text="The story continues.")

        _validate_scene_ready(fragment, require_image=False)
        self.assertFalse(_scene_has_ready_image(fragment))
