...
from bard_core.images.background_removal import remove_background_to_alpha
from bard_core.images.generator import generate_images_for_fragments
from bard_core.images.imagen_provider import generate_imagen_image
...
class ImageAssetTests(TestCase):
    ...
    def test_background_removal_preserves_existing_alpha_png(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            result = remove_background_to_alpha(input_path, output_path)

            self.assertEqual(result, output_path)
            with Image.open(output_path) as saved:
                self.assertEqual(saved.mode, "RGBA")
                self.assertEqual(saved.getpixel((0, 0))[3], 0)
                self.assertEqual(saved.getpixel((1, 1))[3], 255)

    def test_imagen_provider_uses_mocked_gemini_image_and_saves_transparent_png(self) -> None:
        from PIL import Image

        generated = Image.new("RGBA", (8, 8), (0, 0, 0, 0))
        generated.putpixel((4, 4), (220, 20, 30, 255))
        calls: list[dict[str, object]] = []

        class FakePart:
            inline_data = SimpleNamespace(data=b"unused")
        ...
        with TemporaryDirectory() as tmp:
            ...
            asset = ImageAsset(role="subject", label="signal mask", prompt="a signal mask")
            with patch.dict(
                sys.modules,
                {
                    "google": fake_google,
                ...
                result = generate_imagen_image(asset, tmp_path / "asset", settings)

            self.assertEqual(result.provider, "imagen")
            self.assertEqual(result.model, "gemini-2.5-flash-image")
            self.assertEqual(calls[0]["client"]["location"], "global")
            self.assertEqual(calls[1]["model"], "gemini-2.5-flash-image")
            with Image.open(result.local_path or "") as saved:
                self.assertEqual(saved.mode, "RGBA")
                self.assertEqual(saved.getpixel((0, 0))[3], 0)
                self.assertEqual(saved.getpixel((4, 4))[3], 255)
    ...
    def test_stream_sends_fine_mood_cues(self) -> None:
        sent: list[tuple[str, object]] = []
        ...
        fragment = StoryFragment(
            id=1,
            mood="ENERGETIC",
            text="The city listens.",
            mood_cues=[
                {"mood": "SOLO", "start_s": 0, "end_s": 15},
                {"mood": "DEEP", "start_s": 15, "end_s": 30},
            ],
        )
        ...
        with patch.dict(sys.modules, {"pythonosc": fake_pythonosc}):
            with patch("bard_core.transport.osc_sender.time.sleep"):
                stream = ProcessingOscStream("127.0.0.1", 5005, slide_duration_s=30, include_images=False)
                stream.send(fragment)

        mood_messages = [payload for address, payload in sent if address == "/mood"]
        self.assertEqual(mood_messages, [[1, "SOLO", 0.0, 15.0], [1, "DEEP", 15.0, 30.0]])
