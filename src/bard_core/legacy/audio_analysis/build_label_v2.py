...
def main():
    ...
    with open(out_dir / "clap_unified_labels.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(captions) + "\n")

    with open(out_dir / "clap_unified_labelbank.json", "w", encoding="utf-8") as f:
        json.dump(bank, f, ensure_ascii=False, indent=2)

    print(f"Wrote {out_dir / 'clap_unified_labels.txt'} ({len(captions)} captions, max {args.max_chars} chars)")
    print(f"Wrote {out_dir / 'clap_unified_labelbank.json'} ({len(bank)} items)")
    print("\nPreview:")
    for c in captions[:6]:
        print(" -", c)
